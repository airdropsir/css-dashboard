...
// NEW FUNCTION: Returns detailed breakdown for a specific cell
export const getCalculationDetails = (record: Record, mode: CalcMode, colIndex: number, isWeighted: boolean) => {
    ...
    // 1. Identify Raw Points contributing to this cell
    let startIdx = 0; 
    let endIdx = 12;
...
export const calculateWeekStats = (weekRecords: Record[], config: Config, profiles: Profile[], mode: CalcMode, isWeighted: boolean): WeekStats => {
            ...
            if (isWeighted) {
                if (mode === 'daily') {
                    ...
                    for (let k = start; k < end; k++) {
                        if (!isNaN(values[k])) validCountInShift++; // values maps 1:1 in 2h mode
                    }
    ...
    return {
        ...
        counts: {
            total: Math.round(totalWeight), // If weighted, this is roughly Total Tonnage Processed
